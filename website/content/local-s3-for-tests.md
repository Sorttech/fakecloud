...
Now `PutObject` fires the Lambda for real — not a stub. fakecloud runs Lambda code in real runtime containers across 23 runtimes. `CompleteMultipartUpload` fires its own `s3:ObjectCreated:CompleteMultipartUpload` event with the full-object checksum attached. Records carry the same fields AWS sends — `configurationId`, `ownerIdentity`, URL-encoded `key`, `sequencer`, and `versionId` on versioned buckets — so a handler written against real S3 events needs no changes.
