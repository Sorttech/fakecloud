...
- **CORS** — `PutBucketCors` rules drive real browser CORS: the `OPTIONS` preflight returns `Access-Control-Allow-Origin/-Methods/-Headers` and `Access-Control-Max-Age`, and an actual request carrying `Origin` gets `Access-Control-Allow-Origin` plus `Access-Control-Expose-Headers`. Any response evaluated against a bucket's CORS config also carries `Vary: Origin, Access-Control-Request-Headers, Access-Control-Request-Method`, so a cache cannot replay one origin's response (or an `Origin`-less, header-free one) to another origin.
