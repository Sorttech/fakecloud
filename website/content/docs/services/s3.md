...
- **CORS** — `PutBucketCors` rules drive real browser CORS: the `OPTIONS` preflight returns `Access-Control-Allow-Origin/-Methods/-Headers` and `Access-Control-Max-Age`, and an actual request carrying `Origin` gets `Access-Control-Allow-Origin` plus `Access-Control-Expose-Headers`. Any response evaluated against a bucket's CORS config carries `Vary: Origin, Access-Control-Request-Headers, Access-Control-Request-Method` — successes, error responses such as a cacheable 404 `NoSuchKey`, and a rejected preflight alike — so a cache cannot replay one origin's response (or an `Origin`-less, header-free one) to another origin. A bucket with no CORS config at all is origin-independent and gets no `Vary`.
