...
- **Versioning** — enable/suspend, list object versions, delete specific versions. A delete on a suspended bucket stacks a `null`-id delete marker and keeps the earlier versions, as on AWS.
...
- **CORS** — `PutBucketCors` rules drive real browser CORS: the `OPTIONS` preflight is matched on origin, `Access-Control-Request-Method` and every header in `Access-Control-Request-Headers` before returning `Access-Control-Allow-Origin/-Methods/-Headers` and `Access-Control-Max-Age`, and an actual request carrying `Origin` gets `Access-Control-Allow-Origin` plus `Access-Control-Expose-Headers`. Actual requests are matched on method as well as origin, so a rule allowing only `GET` hands no allow-origin to a `DELETE`. Multipart operations are CORS-evaluated like any other request, so browser multipart upload works. Errors are evaluated too, so a 404 `NoSuchKey` fetched by an allowed origin carries `Access-Control-Allow-Origin` and reaches the caller as a real 404 rather than an opaque CORS failure. Every response evaluated against a bucket's CORS config — successes, errors, and a rejected preflight alike — carries `Vary: Origin, Access-Control-Request-Headers, Access-Control-Request-Method`, so a cache cannot reuse one origin's response for a different origin. As on S3, a request without `Origin` is not CORS-evaluated and gets neither header: an actual request is served normally, and a preflight is rejected with `400 InvalidRequest` ("Insufficient information. Origin request header needed."), distinct from the `403` a disallowed origin gets. A bucket with no CORS config answers identically for every origin and so gets no `Vary`. A CORS rule missing `AllowedMethods` or `AllowedOrigins` is rejected at write time rather than silently matching nothing.
