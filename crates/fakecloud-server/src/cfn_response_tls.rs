...
//! publicly trusted. Handlers verify it rather than skipping verification: the
//! PEM is written to disk (`publish_ca_bundle`) and the Lambda runtime copies
//! it into each function container and points the runtimes' trust settings at
//! it. Copied rather than bind-mounted, because fakecloud commonly runs in a
//! container while Lambda containers are its siblings, and a file inside
//! fakecloud's container cannot be mounted into theirs.
...
/// Generate a self-signed certificate covering the names a Lambda container
/// might dial fakecloud by, returning both the rustls server config and the PEM
/// to hand to Lambda containers so they can verify this endpoint.
