//! Model-driven input validation for Amazon Transcribe.
//!
//! AWS validates request members against the Smithy model's `required`,
//! `length`, `range`, `enum` and `pattern` constraints and returns a
//! `BadRequestException` before any business logic runs. This module reproduces
//! that: the Transcribe Smithy model is embedded and parsed once, and
...
/// `BadRequestException`, Transcribe's canonical request-validation shape.
...
mod tests {
    ...
    fn missing_required_member_is_rejected() {
        let err = validate_input("GetTranscriptionJob", &json!({})).unwrap_err();
        assert!(err.contains("TranscriptionJobName"));
    }
    ...
    fn valid_input_passes() {
        assert!(validate_input(
            "GetTranscriptionJob",
            &json!({ "TranscriptionJobName": "my-job" })
        )
        .is_ok());
    }
    ...
    fn bad_enum_is_rejected() {
        let err = validate_input(
            "CreateVocabulary",
            &json!({ "VocabularyName": "v", "LanguageCode": "xx-XX" }),
        )
        .unwrap_err();
    ...
    fn bad_pattern_is_rejected() {
        // A name with a space violates the `^[0-9a-zA-Z._-]+$` pattern.
        let err = validate_input(
            "GetTranscriptionJob",
            &json!({ "TranscriptionJobName": "has space" }),
        )
        .unwrap_err();
